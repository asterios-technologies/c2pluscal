(**
    Generates PlusCal code from a given  C program.
...
(**
  Converts an unary operation to its corresponding PlusCal representation.
...
let rec pc_of_exp = function
  ...
  | StartOf lval ->
    (*[AddrOf] takes the address of an lvalue
      ...
      Our PlusCal representation treats arrays as variables, so we treat 
      [StartOf] as [AddrOf].*)
    pc_of_lval lval
  ...
  | e -> Result.error 
      (Printf.sprintf "pc_of_exp: Expression not treated: %s\n" (exp_to_str e))


(**
  Translates a memory access expression [e] into a PlusCal expression.
  It uses the [pc_of_exp] function to convert the expression node [e.enode]
   into a PlusCal expression.
...
and pc_of_mec_access (e: exp) = Result.bind (pc_of_exp e.enode) (fun pc_exp_mem ->
  (match pc_exp_mem with
    ...
    | PBinop (PSubPP, PLval (pc_lval), PLval (pc_lval2)) -> 
      Result.ok (PIndex (PLval pc_lval2, pc_lval))
...
and pc_of_lval = function
  ...
  | Mem e, Field(finfo,_) ->
    pc_of_mec_access e |> Result.map (fun pc_lval ->
      PField (finfo.fname, pc_lval))

  (* Converts a lvalue indexed element into a PlusCal index.*)
  | Var vinfo, Index(e,_) ->
    pc_of_exp e.enode |> Result.map (fun pc_exp ->
...
(**
  ...
  @return [Result] containing the translated PlusCal initialization or an error
  message.
*)
...
(**
  Converts a complex initialization type (array, struct, etc) to a PlusCal
  expression.
...
(**
  ...
  @param l list of tuples where each tuple consists of a field offset and an
  initializer.
  @return [Result] containing parsed record initializer or an error message.

  @param l list of tuples where each tuple consists of an index offset and an
  initializer.
...
and handle_record_init l =
  ...
  let record_result = fold_left_result (fun (offs, i) -> 
    match offs with
...
(**
  ...
  @return [Result] containing the list of processed expressions or an error if
  any expression fails to process.
*)
let pc_of_exp_list l = 
  fold_left_result (fun e -> pc_of_exp e.enode) (fun acc e -> acc@[e]) [] l
...
let pc_of_instr = function
  ...
  | Skip _ -> Result.ok ([])
  | i -> Result.error 
      (Printf.sprintf "pc_of_instr: Instr not treated %s" (instr_to_str i))
...
let rec pc_of_stmt = function
  ...
  | Goto (r,_) -> 
      (Result.ok [PGoto (Pretty_utils.to_string Printer.pp_label 
        (List.hd (!r).labels))], 0)
...
class gen_pc (prog: pc_prog ref) = object
  ...
  (*
    ...
    Adds one main process to the pc_prog and one process that initializes 
    global variable.
  *)
  method! vfile f =
    let name = get_file_name() in
    let entry_point = get_entry_point f
    and nb_process =  1 in
    let processus =
      [{pc_process_name="proc";
        pc_process_set="PROCESS";
        pc_process_vars=[];
        pc_process_body=[PAwaitInit;
        PCall (entry_point,[])]}]
    in
      ...
      Cil.DoChildrenPost(fun f ->
        ...
        let glob_init_process =
          {
            ...
            pc_process_body=(List.fold_left
             (* A variable is represented here as a tuple (str, int option).
                The second parameter indicates if the variable is an array, providing 
                its size if it is. *)
        ...
        in
        prog := {!prog with pc_processus = glob_init_process::(!prog).pc_processus};f)


  (*
    Processes global declarations in the CIL AST and updates the program state
    accordingly.
