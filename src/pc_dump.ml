(**
    Dump PlusCal specification in `.tla` file.
**)
...
let dump_proc_var out (proc_name: string) (v_name: pc_var) =
  let vname = (var_to_string proc_name v_name) in
  Format.fprintf out "    %s = " vname;
  Format.fprintf out "[loc |-> \"stack\", fp |-> Len(my_stack), offs |-> 0];\n"
...
let dump_arg (proc_name: string) out (v: pc_var) =  
  Format.fprintf out "%s" (var_to_string proc_name v)


(**
  Outputs the string representation of the given
  PlusCal binary operator [binop] to the formatter [out].
  @param out formatter to which the binary operator will be printed.
  @param binop PlusCal binary operator to be printed. It can be one of the following:

  This function does not handle the pointer operations.
*)
let dump_pc_binop out (binop: pc_binop) = 
  match binop with
  ...
  |_ -> Format.eprintf "Error: dump_pc_binop: non-ptr operation expected"


(**
  Outputs the PlusCal representation of a unary operator [unop] to the formatter [out].
  @param out formatter to print to.
  @param unop unary operator to be printed. It can be one of the following:
*)
let dump_pc_unop out (unop: pc_unop) = 
  match unop with
  ...
  | PLval lval ->
    let ptr_string = string_of_pc_lval proc_name lval in
      (match b with
        ...
        | PAddPI -> 
          Format.fprintf out "[loc |-> %s.loc, fp |-> %s.fp, offs |-> %s.offs-"
        ...
        | PSubPP -> 
          (match e2 with
           ...
           | _ -> Format.eprintf "Error: dump_pc_binop_ptr: second lval operand expected")

        | _ -> Format.eprintf "Error: dump_pc_binop_ptr: binop should be ptr operator")
  | _ -> Format.eprintf "Error: dump_pc_binop_ptr: first lval operand expected"
...
(**
  Outputs the PlusCal representation of an expression [expr] to the formatter [out].
...
*)
and string_of_pc_lval (proc_name: string) (lval: pc_lval) = 
  match lval with
...
(**
  ...
  @param info tuple containing the label, procedure name, line number, 
...
let rec dump_pc_instr out (info: dump_info) (instr: pc_instr) =
  ...
  | PIf (e,l1,l2) ->
    ...
    (List.iteri 
      (fun i -> dump_pc_label out
        ((String.concat "" [label; (string_of_int i)]), proc_name, line, add_indent 1 indent)
      ) l1);

    (*Print else if there is one*)
    if List.length l2 > 0 then Format.fprintf out "%selse\n" indent;
    (List.iteri 
      (fun i -> dump_pc_label out
        ((String.concat "" [label;(string_of_int (List.length l1+i))]), proc_name, line, add_indent 1 indent)
      ) l2);
  ...
  | PCall (fname,args) -> 
    Format.fprintf out "call %s(" fname;
    dump_list out args (dump_pc_expr proc_name);
    Format.fprintf out ");\n";

  | PReturn e -> 
    Format.fprintf out "push(ret, ";
    dump_pc_expr proc_name out e;
    Format.fprintf out ");\n";

  | PDecl (e,ptr) -> 
    Format.fprintf out "decl(";
    dump_pc_expr proc_name out e;
    Format.fprintf out ",%s);\n" (ptr_to_string proc_name ptr);

  | PCopy (e,ptr_dst) -> 
    Format.fprintf out "%s := " (ptr_to_string proc_name ptr_dst);
    dump_pc_expr proc_name out e;
    Format.fprintf out ";\n";
  ...
  | PGoto lbl -> 
    Format.fprintf out "goto %s;\n" (remove_last_char lbl); 
    
    (* lbl = "label:", remove the ":"*)
  ...
  | PInitArray (size,ptr) -> 
    Format.fprintf out "call init_array(%d,%s);\n" size (ptr_to_string proc_name ptr)

(**
  Outputs a label for a PlusCal instruction [i] to the output formatter [out].
  @param out formatter to print the label to.
  @param info tuple containing the generated label string, procedure name (unused),
  the line number, and the indentation string.
...
(**
  ...
  @param proc PlusCal process to be dumped, which includes the process name, 
  set, variables, and body.

  The function performs the following steps:
  - Outputs the process header with the process name and set.
  - Outputs the variables section.
  - Outputs the core of the process, iterating over the process body and dumping
    each label.
...
let dump_pc_process out (proc: pc_process) =
  ...
  (List.iteri (fun i -> dump_pc_label out 
      (proc.pc_process_name,proc.pc_process_name,i,(string_of_indent 1))
    ) proc.pc_process_body);
...
let dump_glob_var out (gv_name: pc_var) =
  let v_name = (var_to_string "glob" gv_name) in
  Format.fprintf out "    %s =" v_name;
  Format.fprintf out "[loc |-> \"mem\", fp |-> 0, offs |-> 0];\n"
...
(**
  Outputs a PlusCal specification of a C program [prog] to the given output
  formatter [out].

  @param out output formatter where the program will be translated.
  ...
  - Prints the CONSTANTS section with the constants used in the program.
  - Prints the global variables section, including initialization of memory
...
let dump_prog out (prog: pc_prog) =
  ...
  dump_list out (List.map (fun proc -> proc.pc_process_set) prog.pc_processus)
    (dump_constant);
  Format.fprintf out ", UNDEF";
  if prog.pc_constants != [] then
    begin
      Format.fprintf out ",\n          ";
      dump_list out (List.map (fun (cst_name,_) -> cst_name) prog.pc_constants)
        (dump_constant);
    end;
  Format.fprintf out "\n\n";
  ...
  Format.fprintf out "    idx_seq(stk, ptr) ==\n";
  Format.fprintf out "        IF \"ptr\" \\in DOMAIN ptr THEN\n";
  ...
  Format.fprintf out "    update_stack(stk, val, seq) ==\n";
  Format.fprintf out "        IF seq = <<>> THEN val\n";
  ...
  (* Dumps Invariant written in `.expect` file, if the option `-expect` is given *)
