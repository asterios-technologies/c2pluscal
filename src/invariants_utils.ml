...
(** Represents a local entry with variable name, process name, process ID,
    and expected value. *)
type local_entry = {
...
(**
  ...
  Can be either local or global, in a tuple with the process name and id,
...
(**
  ...
  and returns a result containing either a [Global] or [Local] entry, or an
  error message.
  ...
  The JSON object is expected to have the following fields:
    ...
    - ["proc_to_check"]: Tuple containing a string (process name) and an integer
      (process ID).
    - ["proc_of_var"]: (Optional) Tuple containing a string (process name)
      and an integer (process ID) for local entries.
*)
...
(**
  ...
  @raise [Json_error] if the JSON file does not contain a list of entries or if
