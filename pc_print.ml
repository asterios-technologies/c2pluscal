...
let print_pc_var out ((n,ptr): pc_var) =
  Format.fprintf out "VAR : name %s ; ptr %s ;" n (string_of_bool ptr)
...
let print_prog out (prog: pc_prog) =
  ...
  Format.fprintf out "GLOB VAR : "; (List.iter (print_pc_var out) (List.map (fun (a,_) -> a) prog.pc_glob_var));Format.fprintf out "\n";
